...
- CSSのレイヤー（cascade layer）
  ...
  - 無名のレイヤーが複数回定義された場合、「無名レイヤー1、無名レイヤー2、...」のようなイメージで無名レイヤーが複数作られる。名前付きレイヤーのようにスタイルが追加されない。
  - `@layer` ルールのブロックの外側に定義されているスタイルは、暗黙的なレイヤーとして扱われ、`!important` が付与されていない状態であれば、名前付きレイヤー・無名レイヤーより優先順位が高い。
  - `@layer` ルールをスタイルと同時に定義している状態で、それより後ろに `@layer <レイヤー名1>, <レイヤー名2>` のように優先順位を定義しても適用されない。スタイルありで定義している `@layer` ルールの記載順序により優先順位が決まる。

- margin と padding を％で指定する場合、その基準は inline-width になる。（水平方向であれば幅）（[リンク](https://developer.mozilla.org/en-US/docs/Learn/CSS/Building_blocks/Sizing_items_in_CSS#percentage_margins_and_padding)）
