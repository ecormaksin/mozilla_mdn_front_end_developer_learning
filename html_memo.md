...
- 文字情報に構造を与える。そのことにより、開発者が意図したように、ブラウザーがHTMLドキュメントを表示できる。
...
- 開始・終了のない単一のタグで構成される要素もある。（empty elements / void elements / 空の要素 ← 上記の構造に当てはめると、「内容」がないことから、そのように呼んでいる？）
...
- リンク・アンカー（anchor）: `<a></a>`
  ...
  - `download`: ページを開くのではなく、ダウンロードさせる場合のファイル名を指定する。
- 画像: `<img src="<画像のパス>">`
- `<span></span>`
...
- リンク先を`href`属性で指定する。
  - ファイル システムのパスの考え方と同じ。
    - 同じディレクトリーのファイル: `<ファイル名>`のみ
    - サブディレクトリー内のファイル: `<サブディレクトリー>/<ファイル名>`
    - 別ディレクトリーのファイル： `../<ディレクトリー>/<ファイル名>`
  - ドキュメントの特定の箇所
    - 対象の要素に`id`属性を付与する。
    - 参照する側は`href`へ`<ファイル名>#<idの値>`を設定する。
    - 同じファイル内の特定の箇所の場合は`#<idの値>`のみ
- URLの指定
  - 絶対URL: `<プロトコル(http/https)>://<ドメイン／サーバー名>(/<ディレクトリー>)/<ファイル名>`
    - ファイル名が`index.html`の場合、`/<ファイル名>`は省略可能。
  - 相対URL
    - 前述のファイル システムの考え方
...
ページを閲覧する人すべてにとって、リンクが _アクセス可能_ にする必要がある。

- スクリーン リーダーの利用者は、ページ内のリンクからリンクへジャンプしたり、ページの内容とは別にリンクを読むことを好む。  
  → ページ内リンクを整備する。
- 検索エンジンは、対象ファイルにインデックスを付けるために、リンクのテキストを使う。そのため、何がリンクされるかを効果的に説明するために、リンクのテキストにキーワードを含めるのは良い考え。
- 視覚的なページ閲覧者は、すべての単語を読むよりもむしろ、ページをサッと見る（スキミングする）。そして、リンクのようにページ内の目立つ箇所に目が向けられやすい。説明がしっかりしているリンクの文章は有益だと感じられる。
...
- 他のコツ
  ...
  - リンク テキストの中で"リンク"や"～へのリンク"と書かない。
    - ノイズになってしまうため。スクリーン リーダーはリンクがあることを教えてくれるし、視覚的には違う色で下線が付加されているのでリンクだと分かる。（このリンクのスタイルについて、利用者は慣れているので、通常は崩すべきではない。）
  - リンク テキストをできるだけ短くする。長いとスクリーン リーダーがリンク テキスト全体を解析する必要があるため。
  - 同じリンク テキストを記述する箇所を必要最小限にする。同じリンク テキストが複数箇所ある場合、スクリーン リーダーが同じ内容を複数回読み上げることになってしまう。

#### HTMLではないリソースへリンクする - 明確な手がかりを残す
